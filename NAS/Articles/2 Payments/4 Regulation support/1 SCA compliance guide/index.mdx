...
<TipBox variant="success" title="Get ready for SCA">
...
Even if SCA applies to your business, some transactions fall [outside the scope of SCA](/payments/regulation-support/sca-compliance-guide#SCAcomplianceguide-outofscope), or can benefit from possible [exemptions](/four/payments/regulation-support/sca-compliance-guide#SCAcomplianceguide-exemptions) if the customer's bank approves.
...
Some issuing banks have started soft declining payments that are not SCA-ready (in other words, not authorizing some payments without [3DS authentication](/payments/regulation-support/sca-compliance-guide#3D_Secure_authentication)) in preparation for the above deadlines, so we recommend you support 3DS authentication as soon as possible to avoid any potential issues.
...
In addition, you must include additional parameters in your payment request to clearly identify such transactions as merchant-initiated. Learn more in our [stored card details guide](/payments/accept-payments/use-saved-details/about-stored-card-details).
...
<TipBox variant="success" title="Implement 3DS authentication">

Use our [3D Secure API integration](/payments/regulation-support/sca-compliance-guide#Implement_3DS_with_our_Payments_API) to implement 3DS authentication on your card payments and request SCA exemptions. It supports both 3DS1 and 3DS2.
...
- Card-based digital wallets like [Apple Pay](/payments/payment-methods/apple-pay) and [Google Pay](/payments/payment-methods/google-pay) have multi-factor authentication built in to their payment flows, offering a frictionless checkout experience that supports SCA.

- Many common European payment methods, like [Bancontact](/payments/payment-methods/cards/bancontact), [iDEAL](/payments/payment-methods/i-deal), and [Multibanco](/payments/payment-methods/cash-and-atm-payment/multibanco), should also follow the SCA requirements.
...
Yes, unless [exemptions](/payments/regulation-support/sca-compliance-guide#Possible_SCA_exemptions) apply.
...
`"3ds.enabled": true` `"merchant_initiated": false` And, if it's a Visa card: `"payment_type": "Recurring"`
...
No. These qualify as [merchant-initiated transactions (MITs)](/payments/regulation-support/sca-compliance-guide#Merchant-initiated_transactions) which fall outside the scope of SCA.

</TableCell>
<TableCell>

`"payment_type": "Recurring" "merchant_initiated": true "previous_payment_id" "source.stored": true`
...
`"3ds.enabled": true "merchant_initiated": false`
...
No. These qualify as [MITs](/payments/regulation-support/sca-compliance-guide#Merchant-initiated_transactions), which are out of scope.

</TableCell>
<TableCell>

`"source.stored": true "merchant_initiated": true "previous_payment_id"`
...
No. [MOTO payments](/payments/regulation-support/sca-compliance-guide#MOTO_payments) are out of scope.
...
`"merchant_initiated": true "previous_payment_id"`
...
Yes. However, you can flag such transactions as [MOTO payments](/payments/regulation-support/sca-compliance-guide#SCAcomplianceguide-moto) to identify them as out of scope.
...
For more details about the additional parameters required for payments using saved cards, see our [stored card details guide](/payments/accept-payments/use-saved-details/about-stored-card-details).
...
To implement 3D Secure authentication and comply with SCA, you need to add the following fields to your payment requests. For more details, see our <ExternalLink href="https://api-reference.checkout.com/preview/crusoe/#operation/requestAPaymentOrPayout">API reference</ExternalLink>.
...
<tabs>
<tab title="3DS challenge request">
