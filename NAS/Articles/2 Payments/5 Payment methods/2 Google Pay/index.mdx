...
title: Google Pay
account: NAS
description: Integrate Google Pay so your customers can securely make one-touch payments using any credit or debit card connected to their Google account.
---
...
1.  [Integrate with Google Pay](/payments/payment-methods/google-pay#Step_1:_Integrate_with_Google_Pay)
2.  [Tokenize the Google Pay payment data](/payments/payment-methods/google-pay#Step_2:_Tokenize_the_Google_Pay_payment_data)
3.  [Request the payment](/payments/payment-methods/google-pay#Step_3:_Request_a_payment_using_Google_Pay)
...
<tabs>
<tab title="Google Pay API request (web)">
...
Now you have the token, it's time to authorize the payment. Take the token, and use it in the body of a card token [payment request](/payments/accept-payments/request-a-payment) from your application or website's backend server.
...
To start testing you need to <ExternalLink href="https://www.checkout.com/get-test-account" >create a test account</ExternalLink>.
