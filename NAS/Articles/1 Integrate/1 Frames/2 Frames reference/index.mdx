...
Allows you to use one of the supported languages or a custom object. Refer to the [Frames customization guide](/payments/accept-payments/frames/frames-customization-guide) for more information.
...
Allows you to customize the look and feel of Frames' controlled fields. Refer to the [Frames customization guide](/payments/accept-payments/frames/frames-customization-guide) for more information.
...
When paying out to a card, the [CVV field](/risk-management/card-verification) is not mandatory, so you can initialize Frames with `CVV_HIDDEN` **or** `CVV_OPTIONAL`.
...
```javascript
Frames.init({
  publicKey: "pk_sbox_8ac41c0d-fbcc-4ae3-a771-31ea533a2beb",
...
The country field only accepts [two-letter ISO country codes](/resources/codes/currency-codes).
